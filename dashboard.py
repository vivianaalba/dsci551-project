...
def format_for_table(data):
    """Convert list-of-dicts into [headers] + rows table format."""
    if not data:
        return []
...
table1 = read_csv(countries_path)       # Countries
table2 = read_csv(food_imports_path)    # Food Imports
...
# Columns for current active data
...
with st.sidebar.expander("Step 6 · Run Selected Steps", expanded=False):
    ...
    if st.button("Run Pipeline"):
        # Start from joined data if exists, else original
...
# # ==========================
# st.header("Step 4: Group and/or Aggregate")
...
#     if group_mode == "Group Only":
#         grouped = group_by(current_data, group_col)
#         current_data = [{group_col: key, "rows": len(rows)} for key, rows in grouped.items()]
#         st.success("Simple Grouping applied.")
